...
{
    public class When_the_worldMatrix_is_Notified_of_GameTimeElapsed_and_the_monsters_destination_is_SOUTHish : AAATest
    {
        private WorldMatrix _worldMatrix;
        private Mock<IRnd> _rndMock;

        private Monster _monster1;
        private readonly Point _oldLocation1 = new Point(-1, -50);

        private Monster _monster2;
        private readonly Point _oldLocation2 = new Point(1, -50);
        ...
        public void The_distance_between_monster1_and_the_destination_becomes_smaller()
        {
            ...
            Assert.True(locationOfMonster.Value.X >= -1 && locationOfMonster.Value.Y > -50, string.Format("the new location must lie closer to (0,0), but is now {0}", locationOfMonster));
        }
